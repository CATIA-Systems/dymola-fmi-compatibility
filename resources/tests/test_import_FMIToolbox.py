...
def test_import_FMIToolbox_fmi2(dymola, resources_dir, model, interface_type):
    filename = (
        resources_dir
        / f"FMIToolbox/fmus/FMIToolbox_{tool_version}/2.0/{interface_type}/win64/{model}/{model}.fmu"
    )
...
def test_import_FMIToolbox_fmi3(dymola, resources_dir, model):
    filename = (
        resources_dir
        / f"FMIToolbox/fmus/FMIToolbox_{tool_version}/3.0/cs/x86_64-windows/{model}/{model}.fmu"
    )
