...
@pytest.mark.parametrize(
    ...
    product(["ControlledTemperature", "CoupledClutches", "Rectifier"], ["cs", "me"]),
)
def test_import_MapleSim_fmi2(dymola, resources_dir, model, interface_type):
    filename = (
        resources_dir
        / f"MapleSim/MapleSim_2024/FMI_Export/FMI_2/{interface_type.upper()}"
        / model
        / f"{model}.fmu"
    )
...
@pytest.mark.parametrize(
    "model, interface_type",
    product(["ControlledTemperature", "CoupledClutches", "SlidingCrank"], ["cs", "me"]),
)
def test_import_MapleSim_fmi3(dymola, resources_dir, model, interface_type):
    filename = (
        resources_dir
        / "MapleSim/MapleSim_2024/FMI_Export/FMI_3"
        / model
        / f"{model}.fmu"
    )
