...
def test_import_Activate_fmi2(dymola, resources_dir, model, interface_type):
    filename = (
        resources_dir
        / f"Activate/Altair-Activate/2.0/export/{interface_type}/win64/{model}/{model}.fmu"
    )
...
@pytest.mark.parametrize(
    "model, interface_type",
    product(
        ["periodic_clock", "sinewave_array", "triggered_and_periodic_clock"],
        ["cs", "me"],
    ),
)
def test_import_Activate_fmi3(dymola, resources_dir, model, interface_type):
    filename = (
        resources_dir
        / f"Activate/Altair-Activate/3.0/export/{interface_type}/x86_64-windows/{model}/{model}.fmu"
    )
